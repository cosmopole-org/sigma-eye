...
export default function Call() {
    ...
    return (
        <div className="w-full h-full relative">
            <CallNavbar />
            <div className="grid grid-cols-2 w-full h-full pb-[104px] pt-8 pl-8 pr-8 gap-8">
                {getUsers().map(item => (
                    <Card isPressable key={item.id} style={{ borderRadius: 16 }} shadow='none' className='bg-transparent'>
                        <Avatar className="w-full h-full aspect-square" src={item.avatar} />
                        <p className="text-md text-center w-full mt-2">Edward Kasperian</p>
                    </Card>
                ))}
            </div>
