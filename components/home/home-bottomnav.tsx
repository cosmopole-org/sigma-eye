...
export default function HomeBottomNav() {
    ...
    const onItemClick = (key: string) => () => {
        ...
        switchLoading(true);
        router.replace('/api/home/' + key);
    }
    return (
        <Card className="grid grid-cols-3 fixed bottom-0 left-0 w-full h-[72px] pt-1" style={{ borderRadius: '24px 24px 0px 0px', zIndex: 50 }}>
            <BottomNavItem itemKey="people" selected={selectedState.get({ noproxy: true })} title="People" icon="people" onClick={onItemClick('people')} />
            <BottomNavItem itemKey="city" selected={selectedState.get({ noproxy: true })} title="City" icon="city" onClick={onItemClick('city')} />
